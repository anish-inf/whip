# loopy-computer (the native macOS driver)

The Swift helper behind loopy's `computer_exec` native tier — AX-tree reads,
...
```bash
task driver          # swift build -c release → internal/computer/bin/loopy-computer
task build           # go build (embeds the helper into loopy)
```
...
sticks (TCC binds to the stable path + signature at `~/.loopy/bin/loopy-computer`).

Dev builds are ad-hoc signed. If Gatekeeper quarantines a copied binary:

```bash
xattr -d com.apple.quarantine ~/.loopy/bin/loopy-computer
```

## Wire protocol

Newline-delimited JSON-RPC 2.0 on stdin/stdout. First line is the plain-text
version announcement `loopy-computer/1` (must match Go's
`internal/computer.ProtocolVersion`). Every call carries `params.token`
matching the `LOOPY_COMPUTER_TOKEN` env var the parent sets — the helper
