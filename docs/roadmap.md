...
- [x] ACP agent mode: `whip acp` serves the Agent Client Protocol over stdio (v1, via `github.com/coder/acp-go-sdk`) so editors like Zed drive whip as a subprocess agent — initialize/session new+load+list/prompt/cancel/set_mode, streaming chunks + tool cards (with diffs) + plan/usage/title updates, permission prompts bridged from `tools.Gate` in `ask` mode, sessions persisted to the same SQLite store the TUI resumes. Plan: [`.ai-docs/plans/acp/`](../.ai-docs/plans/acp/README.md)
